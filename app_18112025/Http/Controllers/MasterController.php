    ...
    public function scheduleUpdate(Request $request)
    {
        try {
            ...
            if ($update_for == 'all') {
                ...
                $old_data = (!is_null($inst_code)) ? $list->whereNotNull('inst_code')->where('inst_code', $inst_code)->get() : $list->whereNull('inst_code')->get();

                foreach ($old_data as $schedule) {
                    $schedule->update([
                        'inst_code' => $inst_code,
                        'start_date' => $start_date,
                        'end_date' => $end_date,
            ...
            // auditTrail(authUserId(), "Schedule for {$schedule_type} Updated");
            DB::commit();
