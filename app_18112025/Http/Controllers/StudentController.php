...
class StudentController extends Controller
{
    ...
    public function studentdetails($form_num)
    {
        ...
        $studentData = [
            ...
            'role_id' => 2,
            'part_sem' => $student->enrl_part_sem,
    ...
    public function generateRegNo(Request $request)
    {
        try {
            ...
            foreach ($students as $formid) {
                ...
                $registrationId = $prefix . str_pad($counter, 8, '0', STR_PAD_LEFT);
        ...
        try {
            ...
            $students = DB::table('pharmacy_register_student_final')
                        ->select([
                            ...
                            's_inst_code',
                            DB::raw("CONCAT(
                                UPPER(COALESCE(s_first_name, '')), 
                                ' ', 
                                UPPER(COALESCE(s_middle_name, '')), 
                                ' ', 
                                UPPER(COALESCE(s_last_name, ''))
                            ) as full_name"),
                            's_father_name'
                        ])
