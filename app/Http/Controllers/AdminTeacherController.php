...
class AdminTeacherController extends Controller
{
    ...
    /**
     * @OA\Post(
     *     path="/api/admin/delete-teacher",
     *     tags={"Admin - Teacher"},
     *     summary="Delete teacher information",
     *     description="Deletes teacher information using fn_admin_deleteteacherinfo",
     *     security={{"token": {}}},
     *     @OA\RequestBody(
     *         required=true,
     *         @OA\JsonContent(
     *             required={"teacher_id", "admin_user_id"},
     *             @OA\Property(property="teacher_id", type="integer", example=914),
     *             @OA\Property(property="admin_user_id", type="integer", example=2),
     *             @OA\Property(property="remarks", type="string", example="not need")
     *         )
     *     ),
     *     @OA\Response(response=200, description="Teacher delete processed"),
     *     @OA\Response(response=400, description="Validation failed"),
     *     @OA\Response(response=401, description="Unauthorized"),
     *     @OA\Response(response=500, description="Internal server error")
     * )
    ...
    public function deleteTeacherInfo(Request $request)
    {
        ...
        if ($validator->fails()) {
            Log::error('DELETE TEACHER INFO API - Validation Failed:', [
                'errors' => $validator->errors()->all(),
            ]);
        ...
        try {
            Log::info('Calling fn_admin_deleteteacherinfo with parameters:', [
                'p_teacher_id' => $teacherId,
            ...
            Log::info('OUTPUT from fn_admin_deleteteacherinfo:', [
                'raw_result' => $result,
                'result_json' => $result->result ?? null,
            ]);
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                Log::error('DELETE TEACHER INFO API - JSON parsing error:', [
                    'error' => json_last_error_msg(),
                    'raw' => $result->result,
                ]);
