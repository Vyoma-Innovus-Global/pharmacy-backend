...
class StudentController extends Controller
{
    ...
    public function studentdetails($form_num)
    {
        ...
        $student = Registerstudent::where('s_appl_form_num', $form_num)
            ->leftJoin(
                'diploma_enrollment',
                'diploma_register_student_final.s_appl_form_num',
                '=',
                'diploma_enrollment.enrl_form_num'
            )
            ->first();

        $amount = DB::table('diploma_fees')
            ->where('fees_type', 'ENROLLMENT')
            ->where('anual', $student->s_part_sem)
            ->pluck('fees_amount')
            ->first();

        $studentExtraInfo = DB::table('diploma_student_extraInfo')->where('ei_appl_form_num', $form_num)->first();
    ...
    public function regReportDownload(Request $request)
    {
        try {
            ...
            $students = DB::table('pharmacy_register_student_final')
                ...
                ->whereNotNull('s_appl_reg_no')
                ->get();
            $institute = Institute::where('i_code', $inst_code)
                ->select('i_code','i_name')
                ->first();


            $pdf = PDF::loadView('registration-list', ['count' => $students->count(), 
                            'students' => $students,'sess_yr'=>$sess_yr,'institute'=>$institute])
                            ->setPaper('a4', 'portrait');

            $fileName = 'PHARM-REG_LIST-' . $inst_code . '-' . time() . '.pdf';
            return $pdf->stream($fileName);
        } catch (\Exception $e) {
            return response()->json([
                'status' => 'error',
