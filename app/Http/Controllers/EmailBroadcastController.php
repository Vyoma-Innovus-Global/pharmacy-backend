...
class EmailBroadcastController extends Controller
{
    ...
    public function sendRegistrationCancellationMail(Request $request)
    {
        ...
        $adminUserId = $request->input('adminuserid', $request->input('adminUserId', $request->input('admin_user_id', $request->input('p_adminuserid', $request->input('adminUserID')))));
        ...
        if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
            return response()->json([
                'error' => true,
                'message' => 'Valid student email address is required and could not be found.',
            ], 422);
        }
        ...
        try {
            ...
            if (function_exists('auditTrail')) {
                $userId = function_exists('authUserId') ? authUserId() : ($studentId ?? 0);
